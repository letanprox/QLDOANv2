...
function loadlistChamdiemHuongdan(){
    xhttp.open("GET", "/api/danhsach-chamdiem-huongdan?page="+page_num+"&MaGV="+MaGV+"&textsearch="+textsearch, false);
    xhttp.send();
}
...
function LoadListChamdiemHuongdan(data) {
    ...
    $('#button-bar').append(returnIconHome() + returnNameIndex('Chấm điểm') + returnNameIndex('Hướng dẫn'));
    ...
    // if(String(document.getElementById('input-search').value) == ''){
    //     $('.chose-bar').empty();
    //     $('.chose-bar').append(returnSearchForm('Nhập','Tìm kiếm'));
    // }
...
function LoadChamdiemhuongdan(infosv,infodoan,infodiem,infobaocaofile){
    ...
    $('#button-bar').append(returnIconHome() +returnNameIndex('Chấm điểm') + returnNameIndex('Hướng dẫn') + returnNameIndex('Chấm điểm') +  returnReturnBtn());
    ...
    else baocaofile = '<a href="/qldean/uploads/'+infodoan.Tep+'">' +  infobaocaofile.Tep_Goc + '</a>';
